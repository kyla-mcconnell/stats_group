...
liarData <- read.delim("~/Documents/PhD/Coding/R/Field_DiscStatswithR/Data/The Biggest Liar.dat", header=TRUE)

#My own bigram data from my current project
bigrams = read.table("~/Dropbox/SPR Results Kyla/PhDstuff/Stimuli/Kyla_stimuli_Exp1.csv", sep=",", fill = TRUE, header = TRUE)
...
To test if the covaranice goes in the same direction (positive) or the opposite direction (negative), the two deviations are multiplied together: If the result is positive (either by multiplying two postitives or two negatives), the variables covary positively. If it is negative (one is positive and the other is negative), the variables covary negatively.
...
Solves the problem of standardization by measuing covariance in terms of standard deviation.
...
#chart.Correlation(as.matrix(lemma_metrics_short),  method = "spearman")
```
...
#ggpairs(lemma_metrics_short, upper = list(continuous = wrap('cor', method = "spearman")))
```
...
#pairs.panels(lemma_metrics_short, scale=TRUE, method = "spearman")
```
